...
def build_payload(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "config": {
            ...
            "rawDates": config.get("_raw_dates", {}),
            "datePolicy": config.get("_date_policy", {}),
            "currentStatcastSeason": int(config.get("current_statcast_season", 2026)),
            "historicalStatcastRefresh": config.get("historical_statcast_refresh", "missing_only"),
